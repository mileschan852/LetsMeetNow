...
async function createInvoice(type, userId) {
  const items = {
    ...
    profile:    { title: 'Unlock Profile',    desc: 'Edit your profile / status anytime',    price: 100,  payload: { type: 'profile', userId } },
  }
...
async function handleSuccessfulPayment(payment) {
  ...
  try {
    switch (type) {
      case 'raffle': {
        const raffles = await sb('raffles', 'GET', null, '?status=in.(waiting,active)&order=created_at.desc&limit=1')
        ...
        await sb('raffles', 'PATCH', { tickets_sold: (raffle.tickets_sold || 0) + 1 }, `?id=eq.${raffle.id}`)

        if ((raffle.tickets_sold || 0) + 1 >= raffle.target_tickets) {
          await drawWinner(raffle.id)
        } else {
          await tg('sendMessage', { chat_id: userId, text: `🎟️ Ticket bought! ${raffle.tickets_sold + 1}/${raffle.target_tickets} sold.` })
        }
        break
      }
      ...
      case 'profile': {
        ...
        await tg('sendMessage', { chat_id: userId, text: '✏️ Profile unlocked! You can now edit your profile / status anytime.' })
        break
      }
    }
  } catch (err) {
...
async function drawWinner(raffleId) {
  try {
    ...
    if (!tickets?.length) {
      await sb('raffles', 'PATCH', { status: 'complete', winner_id: null, winner_name: 'No tickets sold' }, `?id=eq.${raffleId}`)
      return
    }
...
async function handleCommand(msg) {
  ...
  if (text.startsWith('/raffle')) {
    ...
    await tg('sendMessage', { chat_id: chatId, text: `✅ Raffle created: ${prize} (target: 10 tickets)` })
    return
  }
  ...
  if (text === '/stats') {
    ...
    await tg('sendMessage', {
      chat_id: chatId,
      text: `🎟️ Raffle #${raffle.id} — ${raffle.tickets_sold}/${raffle.target_tickets}\n${lines.join('\n') || 'No tickets yet'}`,
    })
    return
  }
}
